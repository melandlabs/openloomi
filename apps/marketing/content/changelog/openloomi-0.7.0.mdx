...
OpenLoomi 0.7.0 marks the release of the **AI coworker experience** — a Loomi desktop pet that mirrors your activity, the Loop proactive execution engine running inside the main app, and a sweeping memory intelligence upgrade with relations, consolidation, and the BEAM benchmark. The release also expands agent runtime support with OpenCode and Hermes ACP, ships a cross-platform FileSystem and Obsidian vault scanner, and consolidates the CLI around `openloomi-ctl` and one-shot mode.
...
Two new runtime providers expand the surfaces Loop and the chat agent can execute on:

- **OpenCode CLI Provider**: A native OpenCode CLI runtime provider, with env-selected runtime support so you can pick the default per session
