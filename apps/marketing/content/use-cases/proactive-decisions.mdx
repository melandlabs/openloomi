...
Click any card and the detail panel slides in from the right with the suggested action (e.g. `calendar_rsvp` with `response: "accepted"`) and four explicit buttons — **▶ RUN** spawns the Claude session with the decision prompt, **DRY RUN** previews the prompt without executing, **✓ MARK DONE** records it as handled, **✕ DISMISS** drops it. Nothing fires automatically; you always pick the action.
