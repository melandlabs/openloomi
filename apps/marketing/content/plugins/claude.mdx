...
The pet's context menu exposes **Open Loomi / Settings / Ask Loomi... / Actions (if configured) / THEME (Fox ✓, Capybara) / Quit**. **Ask Loomi...** opens the main chat with an editable task draft. Configured prompt actions from `~/.openloomi/pet-actions.json` send a named task prompt into the same Loomi agent runtime. The theme switch is hot-reload — the file watcher picks up `activeTheme` in `pet-config.json` within ~250 ms, and the bridge never writes these files.
