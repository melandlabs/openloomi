...
export default function DiscordAuthorizedPage() {
  return (
    <Suspense fallback={<DiscordAuthorizingFallback />}>
      <DiscordAuthorizedContent />
    </Suspense>
  );
}

function DiscordAuthorizedContent() {
  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      ...
      try {
        ...
        toast.success(
          t("auth.discordConnected", "Discord connected successfully!"),
        );
    ...
    handleOAuthCallback();
  }, [code, stateParam, guildId, providerError, errorDescription, t]);
        ...
        {isAuthorizing || isProcessing ? (
          <div className="space-y-6">
            ...
            <p className="text-gray-600">
              {t(
                "auth.pleaseWait",
                "Please wait while we connect your Discord server",
              )}
            </p>
          </div>
        ) : error ? (
          ...
          <div className="space-y-6">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">
              {t("auth.discordConnected", "Discord Connected!")}
            </h1>
            <p className="text-gray-600">
...
function DiscordAuthorizingFallback() {
  const { t } = useTranslation();
