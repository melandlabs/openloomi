...
| `AI_PROVIDER_REQUIRED` | Run `loomi-bridge sync-claude-env` if `ANTHROPIC_API_KEY` / `ANTHROPIC_BASE_URL` / `ANTHROPIC_MODEL` are set; otherwise walk the user through the OpenLoomi Desktop Preferences page. |
...
- The bridge reads `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` once,
  locally; never prints the key.
- If the user pastes a key into chat, redact it: do NOT echo it back, and
  tell them to remove it from chat history.
